List<Articolo> archivio = new ArrayList<>();
List<Articolo> cercati = new ArrayList<>();
    ...
    while(err){
        ...
        try{
           ...
           if(scelta<0 || scelta > 6) {
            ...
            IO.println("Numero inaccettabile");
            err = true;
        }
    }
    return scelta;
...
void aggiungiArticolo(){
    ...
    while(err) {
        ...
        if(!Articolo.checkID(id)) {
            IO.println("Id inaccettabile");
            err = true;
        }
    }
    ...
    while(err) {
        err = false;
        dscr = IO.readln("Inserisci la descrizione: ");
        if(!Articolo.checkDescr(dscr)){
            IO.println("Descrizione inaccettabile!");
            err = true;
        }
    }
    ...
    while(err) {
        err = false;
        type = IO.readln("Inserisci il tipo: ");
        if(!Articolo.checkType(type)){
            IO.println("Tipo inaccettabile!");
            err = true;
        }
    }
    ...
    while(err) {
        ...
        try{
            prz = Double.parseDouble(prezzo);
            if(!Articolo.checkPrezzo(prz)){
                IO.println("Prezzo inaccettabile");
                err = true;
            }
        }catch (Exception e){
    ...
    while(err) {
        err = false;
        String dt = IO.readln("Inserisci la data di aggiunta al catalogo: ");
        try{
            data = LocalDate.parse(dt);
            if(!Articolo.checkData(data)){
                IO.println("Data inaccettabile");
                err = true;
            }
        }catch (Exception e){
            IO.println("Data in formato inaccettabile!");
            err = true;
        }
    }

    archivio.add(new Articolo(id, dscr, type, prz, data));
}
...
void modificaArticolo(){
    ...
    if(!archivio.getFirst().getArchivio().contains(id)){
        ...
        try{
            ...
            IO.println("Scelta inaccettabile!");
        }
    }
}
